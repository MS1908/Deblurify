...
from deblur_gan.fpn_mobilenetv2 import FPNHead
...
class FPN(nn.Module):
    ...
    def __init__(self, norm_layer, num_filters=128, pretrained=True):
        super().__init__()
        net = mobilenet_v3_large(pretrained=pretrained)
        self.features = net.features

        self.enc0 = nn.Sequential(*self.features[0:2]) # 16
        self.enc1 = nn.Sequential(*self.features[2:4]) # 24
        self.enc2 = nn.Sequential(*self.features[4:6]) # 40
        self.enc3 = nn.Sequential(*self.features[6:11]) # 80
        self.enc4 = nn.Sequential(*self.features[11:16]) # 160
